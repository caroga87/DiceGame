...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, // lo que el cliente pide en la peticion
        ...
        if (autHeader == null || !autHeader.startsWith("Bearer")){ // verifica si envia un jwt (header de autorizacion) o si envia las credenciales empezando por bearer, si no lo hace se le retorna un error 403
            filterChain.doFilter(request, response);
            return; //
        }
        jwt = autHeader.substring(7); // si envia un jwt, coja a partir del valor 7, sin tener en cuenta la palabra bearer
